...
已获取用于调用API时，用于身份授权的服务账号。详细步骤，参考 [API鉴权](authen_overview)。

**获取资源权限**

服务账号已取得系统中相关资源的访问权限。详细信息，参考 [管理服务账号](/docs/enos/zh_CN/2.0.9/iam/howto/service_account/managing_service_account.html)。
...
使用Java core SDK调用接入和资产等服务，需要安装Device And Asset API Pojo SDK V0.2.3及以上版本。
...
```
**Device and Asset API Pojo SDK** <pojo>
...
Request内置支持Header，Query，RequestBody，Path参数。Request和Response的使用方法，参考 示例3和示例4。
...
### 示例3 - 使用Java core SDK和Device and Asset API SDK（GET方法） 

以下示例为使用Java core SDK和Device and Asset API Pojo SDK调用*GET Asset* API获取资产详细信息（开启API日志功能）：
...
### 示例4 - 使用Java core SDK和Device and Asset API SDK（POST方法） 
